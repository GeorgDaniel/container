...
public class PropertyMappingsHelper {
    ...
    /**
     * Evaluates the property mappings of a boundary definition's properties against the xml fragment representing these
     * properties and uses node template instances for this purpose.
     *
    ...
    private void updateServiceInstanceProperties(final ServiceTemplateInstance serviceInstance,
        ...
        if (Objects.isNull(template.getBoundaryDefinitions()) || Objects.isNull(template.getBoundaryDefinitions().getProperties())
            || Objects.isNull(template.getBoundaryDefinitions().getProperties().getPropertyMappings())) {
